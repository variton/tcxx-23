...
auto unexpected(ErrorType error, std::string msg) {
  return std::unexpected(ErrorInfo<ErrorType>{error, msg});
}
 ...
 * @param target_error Error identifier assigned to the propagated error.
 * @return An unexpected value containing @p target_error and the original
 *         error message.
 *
 * @pre @p result contains an error.
 */
template <typename TargetError, typename T, typename SourceError>
auto propagate(const std::expected<T, ErrorInfo<SourceError>> &result,
 ...
 * @tparam SourceError Source error identifier type.
 * @param result Expected result containing the source error.
 * @return An unexpected value containing the original error identifier and
 *         message.
 *
 * @pre @p result contains an error.
 */
